...
impl TextInput {
    ...
    fn shift_home(&mut self, _: &ShiftHome, _: &mut Window, cx: &mut Context<Self>) {
        let range = self.previous_boundary(self.cursor_offset());
        self.move_to(0, cx);
        self.select_to(range + 1, cx);
    }

    fn shift_end(&mut self, _: &ShiftEnd, _: &mut Window, cx: &mut Context<Self>) {
        let range = self.next_boundary(self.cursor_offset());
        self.move_to(self.content.len(), cx);
        self.select_to(range - 1, cx);
    }
...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        let hsla = gpui::Hsla::from(gpui::rgb(0x868e96));

        let (display_text, text_color) = if content.is_empty() {
            (input.placeholder.clone(), hsla)
        } else {
            (content.clone(), hsla)
        };
